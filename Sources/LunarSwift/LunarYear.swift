...
public class LunarYear: NSObject {
    ...
    static let yearCache = LunarYearCache(capacity: 128)
    ...
    public class func fromYear(lunarYear: Int) -> LunarYear {
        yearCache.value(for: lunarYear) {
            LunarYear(lunarYear: lunarYear)
        }
    }
...
final class LunarYearCache {
    private let capacity: Int
    private let lock = NSLock()
    private var values: [Int: LunarYear] = [:]
    ...
    func value(for year: Int, create: () -> LunarYear) -> LunarYear {
        lock.lock()
        defer { lock.unlock() }

        if let value = values[year] {
            accessOrder.removeAll { $0 == year }
            accessOrder.append(year)
            return value
        }
        ...
        if values.count > capacity {
            let evictedYear = accessOrder.removeFirst()
            values.removeValue(forKey: evictedYear)
        }
        return value
    }
